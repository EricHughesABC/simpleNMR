...
class ProblemToHTML:
    def __init__(self, nmrproblem):
        ...
        self.summary_df = pd.DataFrame(columns = ["predicted", 
                                        "carbon", 
                                        "proton", 
                                        "integral", 
                                        "J Class", 
        ...
        self.emol = self._annotate_rdkit_mol_with_ppm(self.emol, self.c13, self.hsqc, column_id="predicted", protons=False)
        self.cmol = self._annotate_rdkit_mol_with_ppm(self.cmol, self.c13, self.hsqc, column_id="ppm", protons=False)
        self.pmol = self._annotate_rdkit_mol_with_ppm(self.pmol, self.c13, self.hsqc, column_id="ppm", protons=True)
        ...
        self.summary_df = pd.DataFrame(columns = ["atom idx", 
                                                  "predicted", 
                                                  "carbon", 
                                                  "proton", 
                                                  "predicted integral", 
        ...
        # create summary table
        self.summary_df = self._create_summary_table(self.summary_df, self.c13, self.hsqc, self.h1, self.mol_df)

        allcarbons = self.c13.shape[0]
        allcarbonswithprotons = self.c13[self.c13.attached_protons>0].shape[0]

        self.svg_str1 = self._add_atom_class_to_svg_string(self.emol_svg, self.summary_df, allcarbons, 0)
        self.svg_str2 = self._add_atom_class_to_svg_string(self.cmol_svg, self.summary_df, allcarbons, 0)
        self.svg_str3 = self._add_atom_class_to_svg_string(self.pmol_svg, self.summary_df, allcarbonswithprotons, 1)
    ...
    def _annotate_rdkit_mol_with_ppm( self, mol, c13df, hsqcdf, column_id="ppm", protons=False):
    ...
    def _add_atom_class_to_svg_string( self, svg_str, summary_df, num_carbon_groups, num_protons=0):
        """
        ...
        for i, line in enumerate(svglines):
            if "note" in line:
                try:
                    ...
                    start_coords = re.search(r"M\s+([0-9.]+)\s+([0-9.]+)", line).groups()

                    notes[i] = {"class": class_attr, "x": float(start_coords[0]), "y": float(start_coords[1])}
        ...
        for circle in circles:
            ...
            if atom_idx in summary_df["atom idx"].values:
                if summary_df[summary_df["atom idx"] == atom_idx]["predicted integral"].values[0] >= num_protons:
        ...
        labels_df = pd.DataFrame(kmeans.labels_[num_carbon_groups:], columns=["labels"], index=list(notes.keys()))
        labels_df["key"] = ""

        for line_no, atom_idx, k_idx in zip(list(notes.keys()), pcircle_labels, kmeans.labels_[:num_carbon_groups]):
            labels_df.loc[labels_df.labels==k_idx, "key"] = f"{atom_idx.split()[-1]}"

        for idx in labels_df.index:
            svglines[idx] = svglines[idx].replace("note", f"atom {labels_df.loc[idx, 'key']}")
    ...
    def _create_summary_table(self, summary_df, c13, hsqc, h1, mol_df):
        ...
        for idx in summary_df.index:
            ...
            if protons.empty:
                ...
                summary_df.loc[idx, "proton"] = ",".join([f"{p:.2f}" for p in protons.values])
        ...
        for idx in summary_df.index:
            ...
            if protons.empty:
                ...
                if protons.empty:
                    ...
                    summary_df.loc[idx, "integral"] = ", ".join([str(int(p)) for p in h1q.numProtons.values])
                    summary_df.loc[idx, "J Class"] = ", ".join([p for p in h1q.jCouplingClass.values])
                    summary_df.loc[idx, "J Coupling"] = ", ".join([f"[{p}]" for p in h1q.jCouplingVals.values if p != 0])
    ...
    def write_html_report(self):
        ...
        summary_html = template.render( filename=self.nmrproblem.problemDirectory,
        ...
        html_file = os.path.join(self.html_dir, self.nmrproblem.problemDirectory + ".html")
