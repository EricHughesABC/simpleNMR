...
class NMRproblem:
    ...
    def __init__(
        ...
        if self.c13_from_hsqc:
            ...
            ch3ch2_df = self.c13[self.c13.CH3CH2].copy()
            ...
            for idx, ppm in zip(ch3_df.index, ch3_df.c13ppm):
                ...
                closest_match = ch3ch2_df.iloc[
                    (ch3ch2_df["ppm"] - ppm).abs().argsort()[:1]
                ]
        ...
        # print out dataframes h1, c13, hsqc, hmbc, cosy, expected_molecule
        print("h1 dataframe")
        print(self.h1)
        print("c13 dataframe")
        print(self.c13)
        print("hsqc dataframe")
        print(self.hsqc)
        print("hmbc dataframe")
        print(self.hmbc)
        print("cosy dataframe")
        print(self.cosy)
        print("expected molecule dataframe")
        print(self.expected_molecule.molprops_df)
        ...
        for k, df in self.excelsheets.items():
            if k in ["H1_1D", "C13_1D", "H1_Pureshift"]:
                if "ppm" not in df.columns:
                    return False, f"Column 'ppm' not found in {k} sheet"

        for k, df in self.excelsheets.items():
            if k in ["HSQC", "HMBC", "COSY", "NOESY"]:
                if "f1_ppm" not in df.columns or "f2_ppm" not in df.columns:
                    return False, f"Column 'f1_ppm' or 'f2_ppm' not found in {k} sheet"
    ...
    def init_c13_from_hsqc_and_hmbc(self):
        ...
        for idx, ppm in zip(self.c13_df.index, self.c13_df.ppm):
            if (
                not self.c13_df.loc[idx, "Quartenary"]
    ...
    def init_class_from_excel(self, excelFileNameDirName: str, excel_fn=None) -> bool:
        ...
        return_value, return_message = self.tidy_up_excel_data()
        if not return_value:
            if self.qtstarted:
        ...
        if self.c13_df.empty:
            ...
            if {"CH2", "CH3CH2", "Quartenary"}.issubset(self.c13_df.columns):
                ...
                self.c13["CH3CH2"] = self.c13_df[self.c13_df.signaltype == "Compound"][
                    "CH3CH2"
                ].copy()
                self.c13["Quartenary"] = self.c13_df[
                    self.c13_df.signaltype == "Compound"
                ]["Quartenary"].copy()
            ...
            if c13view.shape[0] == mol_dfview.shape[0]:
                ...
                if numprotons == 0:
                    ...
                    for i1 in c13view.index:
                        ...
                        i2 = (
                            mol_dfview["predicted"]
                            .sub(c13view.loc[i1, "ppm"])
                            .abs()
                            .idxmin()
                        )
                        ...
                        print(
                            i1,
                            self.c13.loc[
                                i1, ["ppm", "x", "y", "attached_protons_predicted"]
                            ],
                        )
